...
describe("ZoneCondition", () => {
    ...
    describe("findNextMatch", () => {
        test("Always available: returns the same instance", () => {
            const zc = always
            expect(zc.findNextMatch(et).unixSeconds).toBe(et.unixSeconds)
        })
        ...
        test("Time only: before starts", () => {
            ...
            expect(zc.findNextMatch(t).unixSeconds).toBe(t.unixSeconds + EorzeaTime.HOUR * 2)
        })

        test("Time only: after starts", () => {
            const zc = ZoneCondition.fromMinimized({z: "LIMSA_LOMINSA", t: {s: 8, e: 12}})
            const t = et.getSpecifiedTime(10, 0)
            expect(zc.findNextMatch(t).unixSeconds).toBe(t.unixSeconds + EorzeaTime.HOUR * 22)
        })

        test("Time only: just start -> next day", () => {
            const zc = ZoneCondition.fromMinimized({z: "LIMSA_LOMINSA", t: {s: 8, e: 12}})
            const t = et.getSpecifiedTime(8, 0)
            expect(zc.findNextMatch(t).unixSeconds).toBe(t.unixSeconds + EorzeaTime.DAY)
        })
        ...
        test("Weather only: single", () => {
            ...
            expect(zc.findNextMatch(t).unixSeconds).toBe(u)
        })
    ...
    describe("findWindowEnd", () => {
        test("Always available: returns the same instance", () => {
            const zc = always
            expect(zc.findWindowEnd(et).unixSeconds).toBe(et.unixSeconds)
        })
        ...
        test("Less than ET 1h", () => {
            ...
            const result = zc.findWindowEnd(t)
